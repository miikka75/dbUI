...
describe('backend-helpers - bootTableNames', () => {
  // The boot set every backend loads for a caller. Firebase, Supabase and the dev server each computed
  // this independently, and only Firebase had learned the self-service clause — a restricted member's
  // owner-column views booted permanently empty on the other two (the union/join render path reads
  // dataCache and nothing else fetches them later).
  const schema = { tables: {
    tasks:     { columns: [{ name: 'title', type: 'text' }] },
    refdata:   { columns: [{ name: 'label', type: 'text' }] },
    chore_log: { columns: [{ name: 'owner', type: 'owner' }, { name: 'chore', type: 'text' }] },
    rsvps:     { columns: { owner: { type: 'owner' }, going: 'text' } }   // map-shaped columns too
  } };

  it('unrestricted (null) loads every table', () => {
    assert.deepEqual(H.bootTableNames(schema, null).sort(), ['chore_log', 'refdata', 'rsvps', 'tasks']);
  });

  it('a grant loads the granted tables PLUS every owner-column table', () => {
    assert.deepEqual(H.bootTableNames(schema, ['tasks']).sort(), ['chore_log', 'rsvps', 'tasks']);
  });

  it('a member with NO grant still loads the self-service tables', () => {
    assert.deepEqual(H.bootTableNames(schema, []).sort(), ['chore_log', 'rsvps']);
  });

  it('never invents a table the schema does not declare', () => {
    assert.deepEqual(H.bootTableNames(schema, ['ghost']), ['chore_log', 'rsvps']);
  });

  it('a schema with no owner columns falls back to the plain grant set', () => {
    const plain = { tables: { tasks: { columns: [{ name: 'title' }] }, notes: { columns: [] } } };
    assert.deepEqual(H.bootTableNames(plain, ['tasks']), ['tasks']);
    assert.deepEqual(H.bootTableNames(plain, []), []);
  });

  it('empty / missing schema is safe', () => {
    assert.deepEqual(H.bootTableNames({}, []), []);
    assert.deepEqual(H.bootTableNames(null, []), []);
    assert.deepEqual(H.bootTableNames(null, null), []);
  });
});

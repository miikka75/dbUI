...
const { convert } = require('./bishopric-fi-to-en');
...
const FIXTURE = {
  ...
  tables: {
    ...
    kokoukset__archive: [
      { id: 'mpd0em1m9s7p4', pvm: '2026-05-24', teema: '', vastuussa: '1na', puhe1: 'Kaisu Metsä-Tokila' },
      // `johtaja` is listSwitch'd to `vieraat`: a visiting authority presides under their own name.
      { id: 'mq51hhvpvctik', pvm: '2026-07-05', teema: '', johtaja: 'Michael Koivisto (VJ)', vastuussa: '2na' }
    ],
...
describe('bishopric fi -> en conversion', () => {
  ...
  it('leaves a visiting officer in `presiding` alone — it is a name, not a role', () => {
    // The asymmetry is deliberate: `responsible` is bishopric-only AND a view filters on it, so an
    // unknown value there is an error; `presiding` carries listSwitch -> visitors, so a name belongs.
    const visiting = bundle.tables.meeting_agenda__archive.find((r) => r.id === 'mq51hhvpvctik');
    assert.equal(visiting.presiding, 'Michael Koivisto (VJ)');
    assert.deepEqual(problems, [], 'a visitor presiding is not a problem to report');
  });

  it('still refuses an unknown value in `responsible`, which a view filters on', () => {
    const bad = JSON.parse(JSON.stringify(FIXTURE));
    bad.tables.kokoukset[0].vastuussa = 'ei-tunnettu';
    assert.ok(convert(bad).problems.some((p) => p.includes('unknown value `ei-tunnettu`')));
  });

// storage-pglite.js — PGlite (PostgreSQL-in-WASM) storage adapter, shared by the dev server and the
// browser's local mode.
//
// WHY THIS EXISTS: dev used to enforce access control in JavaScript (dev/server.js), Supabase enforces
// it in RLS, and Firebase in rules — three implementations of one policy, kept honest by comparing their
// source TEXT. This adapter runs the real supabase-schema.sql, verbatim, inside the process that loads
// it, so dev and Supabase stop being two implementations and become one file executed twice.
//
// It presents the same interface as storage-supabase.js / storage-firestore.js (get/put/delete/getAll/
// getMeta/setMeta/_all/_replace/_merge) over the identical kv(store, key, value jsonb) table, so the
// mapping from the app's contract onto storage is shared rather than reinvented — backend-kv.js runs
// unmodified on top of either.
//
// TWO HOSTS, ONE FILE:
//   Node    : require('../storage-pglite') — the dev server (dev/backend-pglite.js). PGlite comes from
//             node_modules, `dataDir` is a directory on disk, and supabase-schema.sql is read with fs.
//   Browser : <script src="/storage-pglite.js"> defines the global createPgliteStorage. The caller
//             (backend-local-pglite.js) passes the PGlite class it imported as an ES module and a
//             `dataDir` of 'idb://<name>', so the cluster lives in IndexedDB. The SQL is fetched.
// Everything else — the policies, the identity shim, the serialization — is the same code either way.
//
// IDENTITY: Supabase authenticates with a JWT; here the host hands over the caller's email and this
// adapter sets `request.jwt.claims` + `set role authenticated` for the statement, exactly as
// dev/test/supabase-rls.test.js does. The policies cannot tell the difference. What the host means by
// "the caller" differs, though, and that difference is the whole security story of each: the dev server
// reads a trusted header on a loopback-only socket, while the browser's local mode has no authority to
// appeal to at all — see backend-local-pglite.js, which says so in the setup screen.
//
...
(function (root) {
  ...
  var STORAGE_MARKER = '-- ---------- Storage bucket';   // the same cut point supabase-rls.test.js uses

  // The policy text, from wherever this host keeps it. Passing `opts.schemaSql` overrides both paths
  // (the RLS tests do that to run a doctored copy); otherwise Node reads the file beside this one and
  // the browser fetches it from the deployed app, resolving through appUrl() so a GitHub Pages project
  // site under /<repo>/ finds it too.
  function loadSchemaSql(options) {
    if (options.schemaSql) return Promise.resolve(options.schemaSql);
    if (isNode) {
      var fs = require('fs');
      var path = require('path');
      return Promise.resolve(fs.readFileSync(path.join(__dirname, 'supabase-schema.sql'), 'utf8'));
    }
    var url = (typeof root.appUrl === 'function') ? root.appUrl('/supabase-schema.sql') : '/supabase-schema.sql';
    return fetch(url).then(function (r) {
      // fetch() resolves on 404, and the host's error page is not a schema: without this the policies
      // would silently not be applied, which on this adapter means a database with no access control.
      if (!r.ok) throw new Error('supabase-schema.sql: ' + r.status + ' from ' + url);
      return r.text();
    });
  }

  // Node's copy of PGlite. It is a DEV dependency, declared in dev/package.json and installed into
  // dev/node_modules -- and this file now sits at the repo root, which has no package.json at all, so a
  // bare require() from here resolves against nothing. Resolve explicitly from dev/ instead, which works
  // whatever the cwd, and fall back to the ordinary lookup in case the package is ever hoisted.
  function nodePglite() {
    var Module = require('module');
    var path = require('path');
    try {
      return Module.createRequire(path.join(__dirname, 'dev', 'package.json'))('@electric-sql/pglite').PGlite;
    } catch (e) {
  ...
  async function createPgliteStorage(opts) {
    const options = opts || {};
    // The browser hands in the class it imported (dynamic ESM import of the vendored dist); Node
    // resolves it from dev/node_modules. Neither path runs in the other's host.
    const PGliteCtor = options.PGlite || nodePglite();
    const db = new PGliteCtor(options.dataDir || undefined);   // undefined = in-memory
    ...
    async function applySchema() {
      ...
      // PERSISTED dataDir (APP_DB=<file>, or the browser's idb:// cluster) this runs again on every boot
      // against a database that already has them -- and an unguarded CREATE ROLE made the second start
      // fail outright with `role "authenticated" already exists`, so the persistent option could be used
      // exactly once. The rest of the bootstrap is already idempotent (`create or replace`, and
      // supabase-schema.sql's own `drop policy if exists`); these two statements were the only ones that
      // were not.
      ...
      const full = await loadSchemaSql(options);
    ...
    // investigation into guesswork, and nothing here is a secret from the person running it -- the dev
    // server is loopback-only, and the browser's local database belongs to whoever is looking at it.
    const denied = (e) => Object.assign(new Error('denied by row-level security'), { cause: e });

    // Whole-row replace (Firestore set() WITHOUT merge) — the write behind setMeta and, for the stores
    // backend-kv.js keeps one document per row (_users, _profiles, _lists, ...), _replace.
    //
    // Unlike put() this DOES read the verdict off `returning`: a replace is only ever issued for a row
    // the caller may also read (their own profile, their own access request, a list the schema opens to
    // them -- see app_can_read), so an empty result here means refused rather than written-but-invisible.
    function replace(store, key, value) {
    ...
    // Shallow top-level merge, via the schema's OWN function rather than a reimplementation, so every
    // host takes exactly the write path production takes: `value || patch` in one statement, SECURITY
    // INVOKER, gated by the same kv policies.
    //
    ...
    function merge(store, key, patch) {
      return asCaller(async function () {
        try {
    ...
    return {
      ...
      // The host calls this before touching any data: the dev server per request (from the trusted
      // X-User header), the browser once per identity change.
      ...
      put(store, key, value) { return merge(store, key, value || {}); },
      ...
      setMeta(key, value) {
        return replace('_meta', key, (typeof value === 'object' && value !== null) ? value : { _value: value });
      },

      // No pending queue -- writes go straight to Postgres (like Firestore/Supabase). Present so the
      // interface is complete for callers that probe for offline support.
      getPending: () => Promise.resolve([]),
      addPending: () => Promise.resolve(),
  ...
  var M = { createPgliteStorage: createPgliteStorage };
  if (isNode) module.exports = M;
  else root.createPgliteStorage = createPgliteStorage;

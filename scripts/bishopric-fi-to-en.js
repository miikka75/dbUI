...
//
//   node scripts/bishopric-fi-to-en.js drive-sync-export-2026-08-27.json
//   node scripts/bishopric-fi-to-en.js <export.json> -o converted.json
//
...
// Which converted column carries a value that is itself renamed.
const VALUE_MAPS = { responsible: BISHOPRIC, presiding: BISHOPRIC };
...
function mapValue(column, value, table, problems) {
  const map = VALUE_MAPS[column];
  if (map && typeof value === 'string' && value !== '') {
    if (!map[value]) { problems.push('unknown value `' + value + '` in ' + table + '.' + column); return value; }
    return map[value];
  }
...
if (require.main === module) {
  ...
  if (!input) {
    console.error('usage: node scripts/bishopric-fi-to-en.js <export.json> [-o converted.json]');
    process.exit(2);
  }
